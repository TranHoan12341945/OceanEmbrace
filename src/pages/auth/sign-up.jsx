...
import { register } from "../../api"; // Import hàm register từ api.js
import {
  Card,
  Input,
  Checkbox,
...
export function SignUp() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const data = await register(password, confirmPassword, fullName, email);
      console.log("Registration successful:", data);
      setError(""); // Xóa lỗi nếu đăng ký thành công
      navigate("/auth/sign-in"); // Điều hướng đến trang đăng nhập
    } catch (error) {
      if (error.response) {
        setError(`Registration failed: ${error.response.data.message || error.response.data}`);
      } else {
