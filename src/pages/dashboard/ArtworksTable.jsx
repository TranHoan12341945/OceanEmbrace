...
export function ArtworksTable() {
  ...
  const [newArtwork, setNewArtwork] = useState({
    ...
    isPublic: true,
    isBuyAvailable: true,
    genreId: "", // updated to store genreId instead of genreName
  });
  ...
  const loadData = async (page) => {
    try {
      ...
      // Fetch artworks and genres
      const [artworksResponse, genresResponse] = await Promise.all([
        fetchArtworks(page, pageSize),
        fetchGenres(),
      ]);

      const { items, totalItems } = artworksResponse;
      setArtworks(items);
  ...
  const handleDeleteClick = async (artworkId) => {
    if (window.confirm("Are you sure you want to delete this artwork?")) {
      try {
        ...
        setArtworks((prevArtworks) =>
          prevArtworks.filter((item) => item.artworkId !== artworkId)
        );
  ...
  const handleCloseAddDialog = () => {
    ...
    setNewArtwork({
      ...
      isPublic: true,
      isBuyAvailable: true,
      genreId: "", // updated to clear genreId
    });
  };
            ...
            <tbody>
              {artworks.map((artwork, key) => {
                ...
                const className = `py-3 px-5 ${
                  key === artworks.length - 1 ? "" : "border-b border-blue-gray-50"
                }`;
                  ...
                  <tr key={artwork.artworkId}>
                    <td className={className}>
                      {isEditing ? (
                        <Input
                          value={artwork.name}
                          onChange={(e) =>
                            handleChange(artwork.artworkId, "name", e.target.value)
                          }
                        />
                      ) : (
                        <Typography
                    ...
                    <td className={className}>
                      {isEditing ? (
                        <Input
                          value={artwork.image}
                          onChange={(e) =>
                            handleChange(artwork.artworkId, "image", e.target.value)
                          }
                        />
                      ) : (
                    ...
                    <td className={className}>
                      {isEditing ? (
                        ...
                        <Chip
                          variant="gradient"
                          color={artwork.isPublic ? "green" : "blue-gray"}
                    ...
                    <td className={className}>
                      {isEditing ? (
                        ...
                        <Chip
                          variant="gradient"
                          color={artwork.isBuyAvailable ? "green" : "red"}
                          value={artwork.isBuyAvailable ? "Available" : "Unavailable"}
                          className="py-0.5 px-2 text-[11px] font-medium w-fit"
                        />
                      )}
                    </td>
                      ...
                      {isEditing ? (
                        <>
                          <Button
                            size="sm"
                            color="green"
                            onClick={() => handleSaveClick(artwork)}
                            className="mr-2"
                          >
                            Save
                          </Button>
                        ...
                        <>
                          <Typography
                            as="a"
                            href="#"
                            onClick={() => handleEditClick(artwork.artworkId)}
                            className="text-xs font-semibold text-blue-gray-600 mr-3"
                          >
                            Edit
                          </Typography>
                          <Typography
                            as="a"
                            href="#"
                            onClick={() => handleDeleteClick(artwork.artworkId)}
      ...
      <Dialog open={isDialogOpen} handler={handleOpenAddDialog}>
        ...
        <DialogBody divider className="space-y-4">
          <Input
            ...
            onChange={(e) => handleNewArtworkChange("name", e.target.value)}
          />
          <Input
            label="Description"
          ...
          <Input
            ...
            onChange={(e) => handleNewArtworkChange("image", e.target.value)}
          />
          <Input
            label="Price"
            type="number"
            value={newArtwork.price}
            onChange={(e) => handleNewArtworkChange("price", parseFloat(e.target.value))}
          />
          <Input
            label="Artist ID"
            type="number"
            value={newArtwork.artistID}
            onChange={(e) => handleNewArtworkChange("artistID", parseInt(e.target.value, 10))}
          />
          <select
            className="p-2 border border-gray-300 rounded-md w-full"
            value={newArtwork.genreId}
            onChange={(e) => handleNewArtworkChange("genreId", parseInt(e.target.value, 10))}
          >
          ...
          <select
            className="p-2 border border-gray-300 rounded-md w-full"
            value={newArtwork.isPublic}
            onChange={(e) => handleNewArtworkChange("isPublic", e.target.value === "true")}
          >
            <option value="true">Public</option>
            <option value="false">Private</option>
          </select>
          <select
            className="p-2 border border-gray-300 rounded-md w-full"
            value={newArtwork.isBuyAvailable}
            onChange={(e) => handleNewArtworkChange("isBuyAvailable", e.target.value === "true")}
          >
            <option value="true">Available</option>
            <option value="false">Unavailable</option>
          </select>
